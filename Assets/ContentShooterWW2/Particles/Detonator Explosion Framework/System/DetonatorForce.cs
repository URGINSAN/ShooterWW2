	...
	override public void Explode()
	{
		...
		if (_explodeDelay <= 0) //if the delayTime is zero
		{
            ...
            foreach (Collider hit in _colliders)
            {
                ...
                if (hit.GetComponent<CharacterHealthController>()) {
                    hit.GetComponent<CharacterHealthController>().PainFromExplosion(transform);
                }
                if (hit.GetComponent<AIHealthController>())
                {
                    hit.GetComponent<AIHealthController>().PainFromExplosion(transform);
                }
                if (hit.GetComponent<HPController>())
                {
                    hit.GetComponent<HPController>().PainFromExplosion(transform);
                }
            }
        ...
        {
            Transform Player = FindObjectOfType<CameraShake>().transform;
            float Dist = Vector3.Distance(Player.position, transform.position);
            //if (Dist < 20)
            //{
            Player.GetComponent<CameraShake>().shakeAmount = Dist / 10;
            Player.GetComponent<CameraShake>().shakeDuration = 2;
            //}
        }
	}
